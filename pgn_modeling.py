...
import collections
from tensorflow.contrib.seq2seq.python.ops.attention_wrapper import _luong_score, _BaseAttentionMechanism, dtypes, \
	layers_core
...
def my_dynamic_decode(decoder,
	...
	with variable_scope.variable_scope(scope, "decoder") as varscope:
		...
		if maximum_iterations is not None:
			maximum_iterations = ops.convert_to_tensor(maximum_iterations, dtype=dtypes.int32,
		...
		def body(time, outputs_ta, state, inputs, finished, sequence_lengths, att_scores, seq_inputs):
			...
			outputs_ta = nest.map_structure(lambda ta, out: ta.write(time, out),
			                                outputs_ta, emit)
			return (
			time + 1, outputs_ta, next_state, next_inputs, next_finished, next_sequence_lengths, att_scores, seq_inputs)
