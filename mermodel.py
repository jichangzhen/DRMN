...
class Model(object):
	...
	def __init__(self, config):
		...
		with tf.name_scope("placeholder"):
			...
			self.query = tf.placeholder(tf.int32, [None, None], name="query")  # batch_size, max_sequence_length
			self.query_lens = tf.placeholder(tf.int32, [None], name="query_lens")  # batch_size
			...
			self.query_mask = tf.sequence_mask(self.input_sentences_lens,
			                                   name="query_mask")  # batch_size, max_sequence_length
		...
		with tf.name_scope("embedding"):
			with tf.device("/cpu:0"):
				...
				if self.config.use_cross_copy or self.config.use_cross_attention or self.config.use_merge:
					self.word_emb = tf.nn.embedding_lookup(self.word_table, self.similar_x, name='word_emb')
					
					if self.config.use_cross_attention:
						self.query_emb = tf.nn.embedding_lookup(self.word_table, self.query, name='query_word_emb')
		...
		with tf.variable_scope("utterance_rnn"):
			if self.config.use_role_embedding:
				tiled_role_embedding = tf.multiply(
					tf.ones([batch_size, max_sentence_num, max_sequence_length, self.config.role_emb],
					        dtype=tf.float32),
					tf.expand_dims(self.role_embedding, axis=2))
			...
			self.word_embedding = tf.multiply(self.word_embedding,
			                                  self.mask)  # (batch_size*max_sentence_num, max_sequence_length, 400)
		...
		if self.config.use_cross_attention:
			with tf.variable_scope("query_rnn"):
				mask_q = tf.sequence_mask(tf.reshape(self.query_lens, [-1]), maxlen=max_sequence_length)
				mask_q = tf.cast(tf.expand_dims(mask_q, axis=-1), dtype=tf.float32)
				self.query_emb = tf.multiply(self.query_emb, mask_q)
				
				cell_fws = MultiRNNCell(
					[CudnnCompatibleLSTMCell(self.config.rnn_hidden_size) for _ in
					 range(self.config.rnn_layer_num)]
				)
				cell_bws = MultiRNNCell(
					[CudnnCompatibleLSTMCell(self.config.rnn_hidden_size) for _ in
					 range(self.config.rnn_layer_num)]
				)
				(query_fw, query_bw), (query_state_fw, query_state_bw) = tf.nn.bidirectional_dynamic_rnn(
					cell_fw=cell_fws, cell_bw=cell_bws, inputs=self.query_emb,
					dtype=tf.float32, sequence_length=tf.reshape(self.query_lens, [-1])
				)
				query_memory_embeddings = tf.concat([query_fw, query_bw], axis=2)
				query_memory_embeddings = tf.multiply(query_memory_embeddings, mask_q)
				self.query_embedd = tf.nn.dropout(query_memory_embeddings, keep_prob=self.dropout_keep_prob,
				                                  name="query_memory_embeddings")
		
		# (batch_size,, max_sequence_length, 600)
		...
		if self.config.use_cross_copy or self.config.use_cross_attention or self.config.use_merge:
			with tf.variable_scope("similar_rnn"):
				...
				sim_memory_embeddings = tf.nn.dropout(sim_memory_embeddings, keep_prob=self.dropout_keep_prob,
				                                      name="sim_memory_embeddings")
				# (batch_size*max_sentence_num, max_sequence_length, 600)
		...
		if self.config.use_cross_attention:
			with tf.variable_scope("memory_networks", reuse=tf.AUTO_REUSE):
				new_sim_memory_embeddings = tf.reshape(sim_memory_embeddings,
				                                       [batch_size, self.config.max_sentence_num, max_similar_length,
				                                        2 * self.config.rnn_hidden_size])
				stk_sim_memory_embeddings = tf.unstack(new_sim_memory_embeddings, axis=1)
				
				i = 0
				for stk_sim in stk_sim_memory_embeddings:
					query_masks = create_attention_mask_from_input_mask(self.query_embedd, self.input_sentences_lens)
					attention_in = attention_layer(
						from_tensor=self.query_embedd,
						to_tensor=stk_sim,
						attention_mask=query_masks,
						num_attention_heads=8,
						size_per_head=2 * self.config.rnn_hidden_size,
						from_seq_length=self.query_lens
					)
					
					sig = tf.layers.dense(attention_in, 1, use_bias=True)
					gate = tf.nn.sigmoid(sig)
					
					if i > 0:
						mem = gate * attention_in + (1 - gate) * mem
						skl_mem = tf.add(tf.layers.dense(mem, 2 * self.config.rnn_hidden_size, activation=tf.nn.relu),
						                 tf.layers.dense(skl_mem, 2 * self.config.rnn_hidden_size,
						                                 activation=tf.nn.relu))
					else:
						mem = attention_in
						skl_mem = tf.add(tf.layers.dense(mem, 2 * self.config.rnn_hidden_size, activation=tf.nn.relu),
						                 tf.layers.dense(self.query_embedd, 2 * self.config.rnn_hidden_size,
						                                 activation=tf.nn.relu))
					
					cell_fwm = MultiRNNCell(
						[CudnnCompatibleLSTMCell(self.config.rnn_hidden_size) for _ in
						 range(self.config.rnn_layer_num)]
					)
					cell_bwm = MultiRNNCell(
						[CudnnCompatibleLSTMCell(self.config.rnn_hidden_size) for _ in
						 range(self.config.rnn_layer_num)]
					)
					
					(output_fw, output_bw), (output_state_fw, output_state_bw) = tf.nn.bidirectional_dynamic_rnn(
						cell_fw=cell_fwm, cell_bw=cell_bwm, inputs=skl_mem,
						dtype=tf.float32, sequence_length=self.query_lens
					)
					skl_mem += tf.nn.dropout(tf.concat([output_fw, output_bw], axis=2), self.dropout_keep_prob)
					
					i += 1
			
			self.skl_mem = skl_mem
		...
		with tf.variable_scope("dialogue_rnn"):
			...
			if self.config.use_cross_copy or self.config.use_cross_attention:
			...
			outputs = tf.concat(outputs, axis=2)  # (?, ?, 600)
			
			self.sample_hidden_states = tf.multiply(outputs, self.mask)
			self.sample_hidden_states = tf.nn.dropout(self.sample_hidden_states, keep_prob=self.dropout_keep_prob)
		...
		with tf.variable_scope("transformer"):
			attention_mask = create_attention_mask_from_input_mask(from_tensor=self.sample_hidden_states,
			                                                       to_mask=self.input_sample_mask)
			
			self.all_encoder_layers = transformer_model(input_tensor=self.sample_hidden_states,
			...
			self.sample_hidden_states = self.all_encoder_layers[-1]
			self.sample_hidden_states = tf.nn.dropout(self.sample_hidden_states, keep_prob=self.dropout_keep_prob)
		
		with tf.variable_scope("sim_transformer"):
			...
			self.similar_hidden_states = self.sim_encoder_layers[-1]
			self.similar_hidden_states = tf.nn.dropout(self.similar_hidden_states, keep_prob=self.dropout_keep_prob)
			
			self.encoder_outputs = self.sample_hidden_states
			self.sim_encoder_outputs = self.similar_hidden_states
		...
		with tf.variable_scope("Decoder"):
			...
			if self.config.use_cross_attention:
				with tf.variable_scope("cross_memory"):
					cross_attention_mechanism = pgn_modeling.MyLuongAttention(2 * self.config.rnn_hidden_size,
					                                                          memory=self.skl_mem,
					                                                          memory_sequence_length=self.query_lens,
					                                                          scale=True)
					
					self.cross_decoder_cell_wrapper = pgn_modeling.MyAttentionWrapper(self.decoder_cell,
					                                                                  cross_attention_mechanism,
					                                                                  attention_layer_size=self.config.rnn_hidden_size * 2,
					                                                                  name="cross_attention_mechanism")
			...
			with tf.variable_scope("attention_decoder"):
				...
				if self.config.use_cross_copy or self.config.use_merge:
				...
				if self.config.use_cross_attention :
					self.cro_decoder_initial_state = self.cross_decoder_cell_wrapper.zero_state(self.batch_size,
					                                                                            tf.float32)
					cro_train_decoder = pgn_modeling.MyBasicDecoder(self.cross_decoder_cell_wrapper,
					                                                train_helper,
					                                                self.cro_decoder_initial_state)
					self.cro_train_outputs, _, _, cro_attn_dists, cro_seq_inputs = pgn_modeling.my_dynamic_decode(
						cro_train_decoder,
						output_time_major=True,
						swap_memory=True)
				...
				if self.config.use_transformer_linear_projection:
					...
					if self.config.use_cross_attention:
						cro_dec = self.transformer_projection_layer(
							tf.nn.dropout(self.cro_train_outputs.rnn_output, keep_prob=self.dropout_keep_prob))
						cro_dec = tf.transpose(cro_dec, [1, 0, 2])
						cro_weights = tf.transpose(self.word_table)  # (300, 20001)
						self.cro_seq2seq_logits = tf.einsum('ntd,dk->ntk', cro_dec,
						                                    cro_weights)  # (N, T2, vocab_size) #(?, ?, 20001)
					...
					if self.config.use_cross_attention:
						self.cro_seq2seq_logits = self.projection_layer_pointer(
							tf.nn.dropout(self.cro_train_outputs.rnn_output, keep_prob=self.dropout_keep_prob))
						self.cro_seq2seq_logits = tf.transpose(self.cro_seq2seq_logits, [1, 0, 2])  # (32, ?, 20001)
						
				if self.config.use_cross_attention:
					vocab_dists = tf.nn.softmax(self.seq2seq_logits + self.cro_seq2seq_logits)  # (32, ?, 20001)
				elif self.config.use_merge:
					vocab_dists = tf.nn.softmax(self.seq2seq_logits + self.sim_seq2seq_logits)  # (32, ?, 20001)
				else:
					vocab_dists = tf.nn.softmax(self.seq2seq_logits)
			...
			with tf.variable_scope("decoder_loss"):
				if self.config.use_pointer_loss:
					loss_per_step = []
			...
			with tf.variable_scope("infer_decoder"):
				...
				while i < self.config.max_decoder_steps:
					...
					if self.config.pointer_generate:
						infer_final_dists = self._calc_final_dist(infer_vocab_dists, re_weighted_infer_attn_dists,
						                                          infer_p_gens)
						
						if self.config.use_cross_copy:
							infer_final_dists = self.sim_calc_final_dist(infer_final_dists,
							                                             re_weighted_sim_infer_attn_dists,
							                                             sim_infer_p_gens)
					else:
